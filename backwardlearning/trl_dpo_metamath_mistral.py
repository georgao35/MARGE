...
if __name__ == "__main__":
    ...
    def grreinforce_process(row):
        ...
        prompt = prompt_template.format(query=row["query"], state=row["prompt"].removeprefix(row["query"]))
        row["prompt"] = prompt
        row["chosen"] = row["response"][0] + "</s>"
        row["rejected"] = row["response"][1] + "</s>"
    ...
    def standard_dpo_process(row):
        ...
        row["chosen"] = row["prompt"].removeprefix(row["query"]) + row["chosen"][-1]['content'] + "</s>"
        row["rejected"] = row["prompt"].removeprefix(row["query"]) + row["rejected"][-1]['content'] + "</s>"
