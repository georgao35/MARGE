...
from backwardlearning.stepdpo_trainer import StepDPOTrainer, QStarTrainer, QStarTrainerSequential
from backwardlearning.trainer.grreinforce_trainer import (
    GroupRelativeReinforceTrainer,
    GRReinforceTrainer,
    GRBatchReinforceTrainer,
    GRPOTrainer,
)
...
if __name__ == "__main__":
    ...
    def qstar_process(row):
        ...
        row["prompt"] = prompt
        row["chosen"] = row["chosen"][-1]['content'] + "<|im_end|>\n"
        row["rejected"] = row["rejected"][-1]['content'] + "<|im_end|>\n"
        return row
    ...
    def grreinforce_process(row):
        ...
        prompt = prompt_template.format(query=row["query"], state=row["prompt"].removeprefix(row["query"]))
        row["prompt"] = prompt
        row["chosen"] = row["response"][0] + "<|im_end|>"
        row["rejected"] = row["response"][1] + "<|im_end|>"
    ...
    def standard_dpo_process(row):
        ...
        row["chosen"] = "<|im_start|>assistant\n" + row["prompt"].removeprefix(row["query"]) + row["chosen"][-1]['content'] + "<|im_end|>\n"
        row["rejected"] = "<|im_start|>assistant\n" + row["prompt"].removeprefix(row["query"]) + row["rejected"][-1]['content'] + "<|im_end|>\n"
    ...
    elif args.method in ["grreinforce", "grpo"]:
        ds = load_dataset("json", data_files=args.dataset_name.split(','))
        if args.sanity_check:
            for key in ds:
                ds[key] = ds[key].select(range(50))
    ...
    if args.method == "qstar" or args.method == "dpo":
        if args.sequential:
            with init_context:
                trainer = QStarTrainerSequential(
                    model,
                    model_ref,
                    args=training_args,
        ...
        if args.sequential:
            with init_context:
                trainer = GRBatchReinforceTrainer(
                    model,
                    model_ref,
                    args=training_args,
                    train_dataset=ds["train"],
