...
def extract_answer_custom_with_idx(pred_str, use_last_number=True, use_choice=False, return_pred_length=False):
    pred_str = pred_str.replace("\u043a\u0438", "")
    ...
    if pred != "" and pred[-1] == "/":
        pred = pred[:-1]
    ridx = pred_str.rfind(pred)
    if return_pred_length:
        pred_length = len(pred)
        # pred = strip_string(pred, skip_unit=False)
        return pred, ridx, pred_length
    pred = strip_string(pred, skip_unit=False)
    return pred, ridx
